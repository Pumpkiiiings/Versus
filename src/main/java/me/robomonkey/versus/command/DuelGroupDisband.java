...
public class DuelGroupDisband extends AbstractCommand {
    ...
    public void callCommand(CommandSender sender, String[] args) {
        ...
        if (!party.isLeader(player.getUniqueId())) {
            player.sendMessage(MessageUtil.color(me.robomonkey.versus.settings.Settings.getMessage(me.robomonkey.versus.settings.Setting.PARTY_NOT_LEADER_DISBAND)));
            return;
        }
        
        pm.disbandParty(party);
        player.sendMessage(MessageUtil.color(me.robomonkey.versus.settings.Settings.getMessage(me.robomonkey.versus.settings.Setting.PARTY_DISBANDED)));
    }
