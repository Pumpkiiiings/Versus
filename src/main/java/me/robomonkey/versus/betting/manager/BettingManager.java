...
public class BettingManager {
    ...
    public void startSession(Player p1, Player p2, String arenaName, me.robomonkey.versus.kit.model.Kit kit) {
    ...
    public void endSession(BettingSession session, boolean commenceDuel) {
        ...
        if (commenceDuel && p1 != null && p2 != null) {
            ...
            // Start duel
            me.robomonkey.versus.arena.model.Arena desiredArena = null;
            if (session.getArenaName() != null) {
                desiredArena = me.robomonkey.versus.arena.manager.ArenaManager.getInstance().getArena(session.getArenaName());
            }
            
            boolean queueIt = false;
            if (desiredArena != null) {
                if (!me.robomonkey.versus.arena.manager.ArenaManager.getInstance().getAvailableArenas().contains(desiredArena)) {
                    queueIt = true;
                }
            } else {
                desiredArena = me.robomonkey.versus.arena.manager.ArenaManager.getInstance().getAvailableArena();
                if (desiredArena == null) {
                    queueIt = true;
                }
            }
            ...
            if (queueIt) {
                ...
                me.robomonkey.versus.duel.model.Request queuedReq = new me.robomonkey.versus.duel.model.Request(p2, p1, 0.0, session.getArenaName(), session.getKit());
                queuedReq.setBettingSession(session);
                me.robomonkey.versus.duel.manager.RequestManager.getInstance().placeInQueue(queuedReq);
            } else {
            ...
            if (p1 != null) p1.sendMessage(me.robomonkey.versus.config.model.Settings.getMessage(me.robomonkey.versus.config.model.Setting.BET_CANCELLED));
            if (p2 != null) p2.sendMessage(me.robomonkey.versus.config.model.Settings.getMessage(me.robomonkey.versus.config.model.Setting.BET_CANCELLED));
        }
    }
