...
public class DuelGroupDeclineDuel extends AbstractCommand {
    ...
    public void callCommand(CommandSender sender, String[] args) {
        ...
        if (party == null || !party.isLeader(player.getUniqueId())) {
            player.sendMessage(MessageUtil.color(me.robomonkey.versus.config.model.Settings.getMessage(me.robomonkey.versus.config.model.Setting.PARTY_NOT_LEADER_DECLINE_DUEL)));
            return;
        }
        ...
        party.broadcast(me.robomonkey.versus.config.model.Settings.getMessage(me.robomonkey.versus.config.model.Setting.PARTY_DUEL_DECLINED).replace("%player%", requesting.getName()));
        
        Party targetParty = pm.getParty(requesting);
        if (targetParty != null) {
            targetParty.broadcast(me.robomonkey.versus.config.model.Settings.getMessage(me.robomonkey.versus.config.model.Setting.PARTY_DUEL_DECLINED_OTHER).replace("%player%", player.getName()));
        }
    }
