...
public class VictoryEffectsMenu {
    ...
    public VictoryEffectsMenu(Player viewer) {
        this.viewer = viewer;
        this.menu = Versus.spiGUI.create(MessageUtil.color("&bEfectos de Victoria"), 3);
        loadEffects();
    }
    ...
    private void loadEffects() {
        ...
        String currentEffect = stats != null ? stats.getActiveVictoryEffect() : "NONE";

        for (VictoryEffect effect : VictoryEffect.values()) {
            ...
            SGButton btn = new SGButton(icon).withListener(e -> {
                if (stats != null) {
                    stats.setActiveVictoryEffect(effect.name());
